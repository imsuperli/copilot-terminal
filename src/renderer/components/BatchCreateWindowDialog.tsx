        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div>
            <button
              ...
              disabled={isScanning}
              className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg bg-[rgb(var(--primary))] text-[rgb(var(--primary-foreground))] font-medium hover:opacity-90 transition-opacity disabled:opacity-50"
            >
              <FolderOpen className="h-5 w-5" />
              <span>{isScanning ? '扫描中...' : '选择文件夹'}</span>
            </button>
            <p className="mt-2 text-sm text-[rgb(var(--muted-foreground))]">
              选择一个文件夹，将自动扫描其中的所有一级子文件夹（不包括以.开头的文件夹）
            </p>
          </div>

          {/* Parent path */}
          {parentPath && (
            <div className="p-3 rounded-lg bg-[rgb(var(--accent))]">
              <p className="text-sm text-[rgb(var(--muted-foreground))]">扫描路径：</p>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium text-[rgb(var(--foreground))]">
                  找到 {folders.length} 个文件夹，已选择 {selectedCount} 个
                </p>
                <button
              ...
              <div className="max-h-[300px] overflow-y-auto space-y-1 border border-[rgb(var(--border))] rounded-lg p-2">
                {folders.map((folder, index) => (
                  <label
                    key={folder.path}
                    className="flex items-center gap-3 p-2 rounded-lg hover:bg-[rgb(var(--accent))] cursor-pointer transition-colors"
                  >
                    <input
                      type="checkbox"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-[rgb(var(--muted-foreground))] truncate font-mono">
                        {folder.path}
                      </p>
                    </div>
                    {folder.selected && (
